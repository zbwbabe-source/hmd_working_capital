...
export async function readCSV(filePath: string, year: number): Promise<FinancialData[]> {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // 계정명 생성 규칙:
    // 패턴 1: 영업활동 > 매출수금 > 합계 > (빈값) => "매출수금"
    // 패턴 2: 영업활동 > 매출수금 > 홍콩마카오 > (빈값) => "매출수금_홍콩마카오"
    // 패턴 3: 영업활동 > 비용 > 광고비 > 합계 => "광고비"
    // 패턴 4: 영업활동 > 비용 > 광고비 > 홍콩마카오 => "광고비_홍콩마카오"
    ...
    if (!중분류1) {
      ...
      // 중분류2가 "합계"면 중분류1을 계정명으로
      account = 중분류1;
    } else if (중분류2 === '홍콩마카오' || 중분류2 === '대만') {
      // 중분류2가 지역이면 중분류1_지역
      account = `${중분류1}_${중분류2}`;
    } else {
      // 중분류2가 세부 항목 (광고비, 매장 임차료 등)
      if (!소분류 || 소분류 === '합계') {
        // 소분류가 비어있거나 "합계"면 중분류2를 계정명으로
        account = 중분류2;
      } else {
        // 소분류가 지역이면 중분류2_지역
        account = `${중분류2}_${소분류}`;
