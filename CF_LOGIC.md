...
1. 대만 `매장 임차료`를 매출 변동률에 100% 연동해 재계산
...
대만 규칙:
- `delta = (salesYoYRate - 기준값) / 100`
- 대만 매출 반영계수: `1 + delta * 0.8`
- `최종 임차료 = 기존 임차료 * 대만 매출 반영계수`

대만 의미:
- 대만 임차료는 고정 최소 없이 매출 변동률에 100% 연동
- 대만 매출이 조정되면 동일 비율로 임차료도 함께 증감
...
- 임차료: 홍콩은 `고정 최소 vs 매출 20%` 룰 적용
- 대만 임차료: 대만 매출 변동률에 100% 연동
